...
class ImageNet32SimCLRDataset(Dataset):
    def __init__(self, npz_files, transform=None, subset_size=None):
        self.images = []
        ...
        # If a subset size is specified, randomly sample a subset of the data
        if subset_size is not None and subset_size < len(self.images):
            self.images = self._random_subset(self.images, subset_size)
    ...
    def _random_subset(self, data, subset_size):
...
num_updates = 256*50 # 50 epochs for 1.28M images with bs=5000

para_comb = list(itertools.product(seed_list, batch_sizes))
(seed, bs) = para_comb[array_index-1]

num_filters = np.array([64,64,128,256,512]).astype(int)  # [32, 32, 64, 128, 256]  filter numbers for each layer
set_seed(seed)
...
imagenet32_dataset = ImageNet32SimCLRDataset(npz_files=npz_files, transform=transform,subset_size=bs)
...
def train(train_loader, dataloader_val, model, projection_head, optimizer, scheduler, temperature=0.15):
    counter = 0
    while counter < num_updates:
        ...
        print(f'Update [{counter+1}/{num_updates}], Train Loss: {loss.item():.4f}, Val Loss: {val_loss.item():.4f}')
...
torch.save(model,f"/home/mila/z/zixiang.huang/scratch/ps_trash/models/resnet18_simclr_imagenet32_seed{seed}_dataset{bs}.pt")
