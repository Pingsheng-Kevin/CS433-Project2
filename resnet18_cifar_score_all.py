...
# Define ResNet18
model_ = torch.load(f'../scratch/models/resnet18_simclr_cifar100_parameters{num_para}_seed{seed}.pt')

preprocessing = functools.partial(load_preprocess_images, image_size=32)

activations_model = PytorchWrapper(identifier=f'resnet18-cifar-p{num_para}-s{seed}', model=model_ , preprocessing=preprocessing)


model = ModelCommitment(identifier=f'resnet18-cifar-p{num_para}-s{seed}', activations_model=activations_model,
...
with open(f'/home/mila/p/pingsheng.li/scratch/brainscore_data/resnet18_cifar_p{num_para}_s{seed}.npy', 'wb') as f:
