...
lr_list = [1e-2, 3.3e-3, 1e-3, 3.3e-4, 1e-4]

para_comb = list(itertools.product(seed_list, divisor, lr_list))
(seed, d, lr) = para_comb[array_index-1]
...
# projection_head = ProjectionHead(input_dim=num_filters[-1], hidden_dim=1024, output_dim=512).to(device)
...
optimizer = torch.optim.AdamW(list(model.parameters()) + list(projection_head.parameters()), lr=lr, weight_decay=1e-4)
...
torch.save(model,f"/home/mila/z/zixiang.huang/scratch/ps_trash/models/alexnet_simclr_imagenet32_parameters{count_parameters(model)}_seed{seed}_epoch{num_epochs}_lr{lr}.pt")
