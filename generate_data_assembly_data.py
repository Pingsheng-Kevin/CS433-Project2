...
for ds in batch_sizes:
    for seed in seed_list:

        print("data size:", ds, "seed", seed)

        try:

            data_mean, data_std = np.load(f'/home/mila/z/zixiang.huang/scratch/ps_trash/brainscore_data/resnet18_imagenet32_ds{ds}_s{seed}.npy')
            print(data_mean)
            mean_score_list.append(data_mean)
            data_size_list.append(ds)
...
np.save('resnet18_imagenet32_data_meanscore_3seeds.npy', np.array(mean_score_list))
np.save('resnet18_imagenet32_data_size_3seeds.npy', np.array(data_size_list))
